...
public class Websocket extends TextWebSocketHandler{
	...
	public void afterConnectionEstablished(
		...
		System.out.println(accountId+"님이"+attinfo.getClnum()+"번 방에 들어왔습니다!");
		...
		for(int i=0;i<sessionList.size();++i){
			...
			System.out.println("현재 방에 참석중인 인원 수:"+accountList.size());
	...
	private List<String> informAccount(
		...
		while(sessionIds.hasNext()){
			...
			if(chatlistnum==clnum){
				System.out.println("아이디:"+id+", 방이름:"+chatlistnum);
				list.add(id);
			}
		}
		return list;
